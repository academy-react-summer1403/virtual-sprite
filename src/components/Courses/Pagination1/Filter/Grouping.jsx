...
import { filterGrouping } from '@core/services/api/courses/filterGrouping.api';

const Grouping = ({ index }) => {
    const [filterCat, setFilterCat] = useState([]);
    const [openIndexes, setOpenIndexes] = useState(new Set());

    const getFilters = async () => {
        const result = await filterGrouping();
        setFilterCat(result);
    };
    ...
    return (
        <>
            <div
                ...
                <h3 className='text-[16px] text-[#263238]'>دسته بندی‌ ها</h3>
                <span>
                    ...
                    <div className='p-2'>
                        {filterCat.map((contentItem, contentIndex) => (
                            <div key={contentIndex} className='flex items-center mb-2'>
                                <input
                                    type="checkbox"
                                    id={contentItem.techName}
                                    className='mx-2'
                                />
                                <label htmlFor={contentItem.techName} className='text-[14px] text-[#455A64]'>
                                    {contentItem.techName}
                                </label>
                            </div>
                        ))}
                    </div>
